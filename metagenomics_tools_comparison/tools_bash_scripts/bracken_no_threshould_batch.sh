...
#Useage: sbatch bracken_no_threshold_batch.sh krakenDir brackenOutDir krakenDB condaEnv.
: '
Description:Perform Bracken analysis on kraken results files located under
 ...
 settings used are 150bps for sequence length, no threshould on the number of reads, and specify the results to have only the species level.
...
Outputs: For each kraken report file in the directory,  1 report file showing each detected species and data about the reads.
'
...
ls $krakenDir |grep  'fasta.txt'|while read report
do bracken -d $krakenDB -i $krakenDir$report -o $brackenOutDir$report-br.txt -r 150 -l S -t 0;
done
